...
const cardProduct = useProductStore();

import { onMounted } from "vue";

onMounted(() => {
  cardProduct.indexProduct("Car");
});
</script>
...
<template>
    ...
    <h1 class="text-2xl font-bold">
      <RouterLink :to="{ name: 'Cars' }"
        ><span class="cursor-pointer"
          ><span class="text-3xl text-pink-500">Recently added </span> more in
          Vehicles</span
        >
      </RouterLink>
    </h1>
  ...
  <div
    ...
    id="font"
    v-if="cardProduct.products.length"
  >
    <div v-for="productItem in cardProduct.products.slice(-2)" :key="productItem.id">
